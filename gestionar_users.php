        ...
        if ($rol === "usuario" && $familiar_id_vincular > 0) {
            ...
            if ($familiar_id) {
                ...
                $stmtRel = $conexion->prepare("
                    INSERT IGNORE INTO relaciones_usuario_familiar (usuario_id, familiar_id)
                    VALUES (?, ?)
                ");
                $stmtRel->execute([$id, $familiar_id]);
            } else {
...
if ($editar_user && $editar_user["rol"] === "usuario") {
    ...
    try {
        ...
        if ($vinculos_usuario_editar) {
            $parts = [];
            foreach ($vinculos_usuario_editar as $fam) {
                $parts[] = htmlspecialchars($fam["nombre"]) . " (" . htmlspecialchars($fam["email"]) . ")";
                $ids_vinculados[] = (int)$fam["id"];
            }
        ...
        if (!empty($ids_vinculados)) {
            ...
            $stmtDisp->execute($ids_vinculados);
        } else {
            // Si no hay ninguno vinculado, todos los familiares son candidatos
                            ...
                            <div class="form-grid">
                                ...
                                <?php if ($isUsuarioEditado): ?>
                                    ...
                                    <div class="form-group vincular-block">
                                        <label>Familiares vinculados actualmente:</label>
                                        ...
                                        <?php if (!empty($familiares_disponibles)): ?>
                                            <select name="familiar_id_vincular">
                                                <option value="0">-- Selecciona un familiar --</option>
                                                <?php foreach ($familiares_disponibles as $fam): ?>
                                            ...
                                            <small>
                                                Solo se muestran familiares que aún no están vinculados a este usuario.
                                            </small>
                                        <?php else: ?>
                                            <div style="font-size:12px; color:#999;">
                                                No hay familiares disponibles sin vincular.
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                    ...
                                    <div class="form-group foto-block">
                                        ...
                                        <small>Puedes actualizar la fotografía de este usuario.</small>
                                    </div>
                                <?php else: ?>
                                    <!-- Para roles que no son usuario, foto ocupa toda la fila -->
