...
// --- Lógica para obtener todos los usuarios con rol 'profesional' ---
...
<head>
    ...
    <style>
        ...
        /* --- TARJETA (IGUAL QUE TU LISTA_FAMILIARES) --- */
        .familiar-card{
            text-align: center;
            width: 280px;
            padding: 28px;
...
<div class="layout">
    <div class="header">
        ...
        <div class="user-role">Lista de Profesionales</div>
    ...
    <div class="main-section">
        <?php if (!empty($profesionales)): ?>
            <?php foreach ($profesionales as $prof):
                $ruta_foto = resolverRutaFotoPerfil($prof);
            ?>
                <div class="familiar-card"
                     onclick="window.location='chat.php?destinatario_id=<?= htmlspecialchars($prof['id']) ?>'">
                    <img src="<?= htmlspecialchars($ruta_foto) ?>" alt="Perfil de <?= htmlspecialchars($prof['nombre']) ?>">

                    <h2><?= htmlspecialchars($prof['nombre']) ?></h2>
                    <div class="email"><?= htmlspecialchars($prof['email']) ?></div>
